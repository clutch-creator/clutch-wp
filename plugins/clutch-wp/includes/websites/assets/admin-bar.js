(function ($) {
  $(document).ready(async function () {
    if (!ClutchAdminBar || !ClutchAdminBar.restUrl || !ClutchAdminBar.nonce) {
    ...
    async function updateDropdown() {
      try {
        ...
        const promises = websites.map(async website => {
          if (website.deploymentId.startsWith(`${website.projectId}-`)) {
            try {
              ...
              console.warn(`Website ${website.url} is not reachable.`);
            }
        ...
        if (dropdownMenu.children().length === 0) {
          dropdownLink.find('.selected-host').text('No hosts to preview found');
        ...
        try {
          ...
          if (!selectedHost && dropdownMenu.children().length > 0) {
            const firstHost = dropdownMenu.children().first().data('url');
          ...
          if (selectedHost) {
            ...
            dropdownLink.find('.selected-host').text('Previewing: Select Host'); // Default text if no host is selected
          }
          ...
          dropdownLink.find('.selected-host').text('Previewing: Select Host'); // Fallback text on error
          toggleChevronVisibility();
        }
      } catch (error) {
    ...
    async function saveSelectedHost(selectedUrl) {
      try {
        ...
        if (!response.ok) throw new Error('Failed to save selected host');
