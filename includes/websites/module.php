...
function rest_save_selected_host($request) {
	...
	$selected_host = isset($params['selectedHost']) ? esc_url_raw($params['selectedHost']) : '';

	if (empty($selected_host)) {
		return new \WP_Error('invalid_params', 'Invalid selectedHost parameter', ['status' => 400]);
	}
	...
	return rest_ensure_response(['success' => true, 'selectedHost' => $selected_host]);
}
...
add_action('admin_enqueue_scripts', __NAMESPACE__ . '\\enqueue_admin_bar_script');
